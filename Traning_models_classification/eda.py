...
DATA_PATH = os.path.join(os.path.dirname(__file__), '_all_situations_binary.csv')

# Колонки, которые нужно исключить из анализа
EXCLUDE_COLS = ['situation_id', 'Время', 'binary_failure']
...
def plot_correlation_heatmap(df, prefix=''):
    ...
    # Добавляем легенду под тепловой картой
    ax2 = plt.subplot(gs[1])
    ax2.axis('off')
    legend_text = "Легенда:\n" + "\n".join([f"{i}: {name}" for i, name in feature_names.items()])
    ax2.text(0.5, 0.5, legend_text, fontsize=12, ha='center', va='center')
...
def analyze_feature_importance(df, target_col='binary_failure'):
...
# Удаление столбцов с более чем 50% пропусков (исключая специальные колонки)
threshold = 0.5
target_col = 'binary_failure'
cols_to_drop = [col for col in missing[missing/100 > threshold].index if col not in EXCLUDE_COLS]
if cols_to_drop:
    print(f'\nУдаляем {len(cols_to_drop)} столбцов с >50% пропусков: {cols_to_drop}')
    df = df.drop(columns=cols_to_drop)
else:
    print('\nНет столбцов с >50% пропусков (кроме исключенных).')
